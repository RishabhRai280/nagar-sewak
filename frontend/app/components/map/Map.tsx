        ...
        <div>
          <div className="flex items-center gap-2 mb-2">
            <span className={`px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider rounded-full border ${isComplaint ? 'bg-red-50 text-red-600 border-red-100' : 'bg-blue-50 text-blue-600 border-blue-100'}`}>
                 {isComplaint ? "Citizen Complaint" : "Public Project"}
            </span>
          </div>
      ...
      <div className="space-y-6 overflow-y-auto pr-2 custom-scrollbar">
        <div className="flex flex-wrap gap-3 text-sm font-medium">
            ...
            <span className={`px-3 py-1.5 rounded-lg border shadow-sm flex items-center gap-1.5 ${data.severity >= 4 ? 'bg-red-50 text-red-700 border-red-100' : 'bg-yellow-50 text-yellow-700 border-yellow-100'}`}>
                <AlertTriangle size={14} /> Severity: {data.severity}/5
            </span>
            )}
            ...
            <span className="px-3 py-1.5 rounded-lg bg-blue-50 text-blue-700 border border-blue-100 shadow-sm flex items-center gap-1.5">
                <DollarSign size={14} /> Budget: ₹{data.budget.toLocaleString()}
            </span>
            )}
        </div>
        ...
        {isComplaint && data.photoUrl && (
            ...
            <p className="text-sm font-bold text-slate-800 flex items-center gap-2">
                <CheckCircle size={16} className="text-emerald-500" /> Evidence Uploaded
            </p>
  ...
  if (items.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-64 text-center p-6">
        ...
        <p className="text-sm text-slate-400 mt-1">Try adjusting your search or status filters.</p>
      </div>
    );
  }
        ...
        return (
          ...
          >
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide">{isComplaint ? "Complaint" : "Project"}</span>
            </div>
    ...
    <div className="p-6 pt-28 space-y-4 border-b border-white/40 flex-shrink-0 bg-white/40 backdrop-blur-md z-10 sticky top-0">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <h3 className="text-xl font-extrabold text-slate-900">Civic Data</h3>
          </div>
          <motion.button 
            ...
            >
                <div className="flex justify-between items-center pb-6 mb-2">
                    <h3 className="text-2xl font-bold text-slate-900 flex items-center gap-2">
                        <Filter size={24} className="text-blue-600" /> Filters
                    </h3>
                ...
                <div className="space-y-6">
                    <div className="space-y-3">
                        <h4 className="text-sm font-bold text-slate-400 uppercase tracking-wider">Data Layers</h4>
                    ...
                    {showComplaints && (
                        <div className="space-y-3 pt-4 border-t border-slate-200/60">
                            <h4 className="text-sm font-bold text-slate-400 uppercase tracking-wider">Refine Complaints</h4>
                            <div>
