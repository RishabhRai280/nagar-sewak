...
export default function ContractorDashboardComponent() {
    ...
    const [activeTab, setActiveTab] = useState<'overview' | 'available'>('overview');
    ...
    const loadData = async () => {
        ...
        try {
            const [fetchedData, openData] = await Promise.all([
                ...
                // Import this function dynamically or ensure it's exported from api.ts
            ...
            <div className="relative z-10 px-6 pb-12 pt-32 lg:px-12 lg:pb-16 lg:pt-36 max-w-7xl mx-auto">
                ...
                <div className="flex gap-4 mb-8 border-b border-slate-200">
                    <button
                        ...
                        className={`pb-3 px-2 font-bold transition ${activeTab === 'overview' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-slate-500 hover:text-slate-700'}`}
                    >
                        My Projects
                    </button>
                    <button
                        onClick={() => setActiveTab('available')}
                        className={`pb-3 px-2 font-bold transition ${activeTab === 'available' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-slate-500 hover:text-slate-700'}`}
                    >
                ...
                {activeTab === 'overview' ? (
                    ...
                    <div className="bg-white/70 backdrop-blur-xl rounded-3xl shadow-lg border border-white/60 p-8">
                        <div className="flex items-center justify-between mb-6">
                            ...
                            <span className="text-xs font-bold bg-blue-100 text-blue-700 px-3 py-1 rounded-full">{openComplaints.length}</span>
                        </div>
                ...
                {/* Tender Modal */}
                {selectedComplaint && (
                    // Import dynamically to avoid SSR issues if needed, or just use standard import
                    <TenderModalWrapper
