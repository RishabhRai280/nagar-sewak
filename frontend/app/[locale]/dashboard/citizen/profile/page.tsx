...
export default function CitizenProfilePage() {
  const t = useTranslations('dashboard.citizen.profilePage');
      ...
      <main className="flex-1 px-6 pb-12 pt-24 lg:px-10 lg:pb-16 lg:pt-28 relative z-10 overflow-y-auto w-full transition-all duration-300">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 bg-white rounded-xl shadow-sm border border-slate-200 p-6 lg:p-8">
            <h2 className="text-sm font-bold text-slate-500 mb-6 uppercase tracking-widest border-b border-slate-100 pb-2">{t('personalDetails')}</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-y-8 gap-x-12">
              <div>
                <label className="text-xs font-bold text-slate-400 uppercase tracking-wide block mb-1">{t('fullName')}</label>
                <p className="text-lg font-bold text-slate-900 border-b border-slate-100 pb-2">{userData.fullName || t('notProvided')}</p>
              </div>
              <div>
                <label className="text-xs font-bold text-slate-400 uppercase tracking-wide block mb-1">{t('username')}</label>
                <p className="text-lg font-bold text-slate-900 border-b border-slate-100 pb-2">@{userData.username}</p>
              </div>
              <div>
                <label className="text-xs font-bold text-slate-400 uppercase tracking-wide block mb-1">{t('email')}</label>
                <p className="text-lg font-bold text-slate-900 border-b border-slate-100 pb-2">{userData.email}</p>
              </div>
              <div>
                <label className="text-xs font-bold text-slate-400 uppercase tracking-wide block mb-1">{t('phone')}</label>
                <p className="text-lg font-bold text-slate-900 border-b border-slate-100 pb-2">{(userData as any).phone || t('notProvided')}</p>
              </div>
              <div className="md:col-span-2">
                <label className="text-xs font-bold text-slate-400 uppercase tracking-wide block mb-1">{t('address')}</label>
                <p className="text-lg font-bold text-slate-900 border-b border-slate-100 pb-2">{(userData as any).address || t('notProvided')}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 lg:p-8 flex flex-col gap-6">
            <h2 className="text-sm font-bold text-slate-500 mb-0 uppercase tracking-widest border-b border-slate-100 pb-2">{t('impactSummary')}</h2>

            <div className="p-5 bg-white rounded-lg border border-slate-200 border-l-4 border-l-blue-600 shadow-sm">
              <div className="flex justify-between items-start mb-2">
                <h3 className="font-bold text-slate-700 text-sm uppercase">{t('totalReports')}</h3>
                <div className="p-1.5 bg-blue-50 text-blue-600 rounded">
                  <ClipboardList size={18} />
                </div>
              </div>
              <p className="text-3xl font-black text-slate-900">{stats.total}</p>
              <p className="text-xs text-slate-500 font-medium mt-1">{t('submittedToDate')}</p>
            </div>

            <div className="p-5 bg-white rounded-lg border border-slate-200 border-l-4 border-l-emerald-600 shadow-sm">
              <div className="flex justify-between items-start mb-2">
                <h3 className="font-bold text-slate-700 text-sm uppercase">{t('successRate')}</h3>
              ...
              <p className="text-xs text-slate-500 font-medium mt-1">{t('resolutionEfficiency')}</p>
            </div>

            <div className="p-5 bg-white rounded-lg border border-slate-200 border-l-4 border-l-purple-600 shadow-sm">
              <div className="flex justify-between items-start mb-2">
                <h3 className="font-bold text-slate-700 text-sm uppercase">{t('badgeLevel')}</h3>
                <div className="p-1.5 bg-purple-50 text-purple-600 rounded">
                  <Star size={18} />
                </div>
              </div>
              <p className="text-3xl font-black text-slate-900">
                {stats.total >= 10 ? t('gold') :
                  stats.total >= 5 ? t('silver') :
                    stats.total >= 1 ? t('bronze') : t('new')}
              </p>
              <p className="text-xs text-slate-500 font-medium mt-1">{t('communityStatus')}</p>
            </div>
