...
public class DataSeeder {
        ...
        public void seed(ApplicationReadyEvent event) {
                ...
                // --- 1. Users ---
                // Admin
                createUser("admin", "admin@nagar.gov", "Nagar Sewak System Admin", hashedPassword,
                                Set.of(Role.ADMIN, Role.SUPER_ADMIN));

                // Citizens
                User aditya = createUser("citizen_pune_1", "aditya.kulkarni@example.com", "Aditya Kulkarni",
                                hashedPassword, Set.of(Role.CITIZEN));
                User rohan = createUser("citizen_mumbai_1", "rohan.mehta@example.com", "Rohan Mehta", hashedPassword,
                                Set.of(Role.CITIZEN));
                User aman = createUser("citizen_delhi_1", "aman.verma@example.com", "Aman Verma", hashedPassword,
                                Set.of(Role.CITIZEN));
                User suresh = createUser("citizen_pune_2", "suresh.patil@example.com", "Suresh Patil", hashedPassword,
                                Set.of(Role.CITIZEN));

                // Contractors
                User ranjeetUser = createUser("contractor_urbanbuild", "urbanbuild@example.com", "Ranjeet Jat",
                                hashedPassword, Set.of(Role.CONTRACTOR));
                User rishabhUser = createUser("contractor_metroworks", "metroworks@example.com", "Rishabh Rai",
                                hashedPassword, Set.of(Role.CONTRACTOR));
                ...
                projectRepo.saveAll(List.of(karveFlyover, shivajinagarDrain, andheriRoad, dadarLighting, lajpatDrain,
                                rohiniBridge));
                ...
                complaintRepo.saveAll(List.of(c1, c2, c3, c4, c5, c6));
                ...
                createOpenTender(c6, "Rohini Footbridge Structural Strengthening",
                                new BigDecimal("600000"), 30);

                // Contractor Bids (UrbanBuild & MetroWorks)
                // Bids for Karve Road (UrbanBuild vs MetroWorks - MetroWorks wins on price/time
                // usually, but maybe not decided yet)
                createBid(c1, urbanBuild, "Standard Repair - 3 Year Warranty", new BigDecimal("550000"), 18, "PENDING");
                createBid(c1, metroWorks, "Express Repair Phase - 1 Year Warranty", new BigDecimal("480000"), 10,
                                "PENDING");

                // Bids for Rohini (Both bid)
                createBid(c6, urbanBuild, "Advanced Structural Polymer Coating", new BigDecimal("700000"), 35,
                                "PENDING");
                ...
                // This project is already marked completed, so we simulate the accepted tender
                // history
                createAcceptedTender(c4, metroWorks, "Smart LED Installation & Control System",
                                new BigDecimal("3200000"), "COMPLETED");
        ...
        private Complaint createComplaint(String title, String description, int severity, double lat, double lng,
                        User user, String status, String photoFilename, String photoUrls, Project project,
                        Instant resolvedAt) {
        ...
        private void createBid(Complaint complaint, Contractor contractor, String desc, BigDecimal quote, int days,
                        String status) {
                Tender t = new Tender();
                t.setComplaint(complaint);
        ...
        private void createAcceptedTender(Complaint complaint, Contractor contractor, String desc, BigDecimal quote,
                        String status) {
                Tender t = new Tender();
                t.setComplaint(complaint);
                t.setContractor(contractor);
        ...
        private void createRating(User citizen, Contractor contractor, Project project, int score, String comment,
